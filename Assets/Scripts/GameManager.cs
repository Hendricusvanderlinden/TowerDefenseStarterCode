...
public class GameManager : MonoBehaviour
{
    ...
    public void Build(TowerType type, SiteLevel level)
    {
        ...
        if (selectedSite.Level != SiteLevel.Onbebouwd && selectedSite.Level != SiteLevel.level1 && selectedSite.Level != SiteLevel.level2 && selectedSite.Level != SiteLevel.level3)
        {
    ...
    public int GetCost(TowerType type, SiteLevel level, bool selling = false)
    {
        ...
        {
            case TowerType.Archer:
                cost = (level == SiteLevel.level1) ? 50 : (level == SiteLevel.level2) ? 75 : (level == SiteLevel.level3 && !selling) ? 150 : 0;
                break;
            case TowerType.Sword:
                cost = (level == SiteLevel.level1) ? 75 : (level == SiteLevel.level2) ? 100 : (level == SiteLevel.level3 && !selling) ? 200 : 0;
                break;
            case TowerType.Wizard:
                cost = (level == SiteLevel.level1) ? 100 : (level == SiteLevel.level2) ? 125 : (level == SiteLevel.level3 && !selling) ? 250 : 0;
                break;
            default:
